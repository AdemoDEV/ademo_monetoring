...
const URL = "https://www.guizettefamily.com/categorie/pokemon/items-produits/case-detb-et-display-pokemon/";
...
async function checkGuizetteStock(browser) {
    ...
    try {
        console.log("🔄 Accès à GuizetteFamily via Proxy...");
        ...
        await page.waitForSelector(".fl-post-column", { timeout: 60000 });

        console.log("🛠️ Debug: Nombre de produits détectés ->",
            await page.evaluate(() => document.querySelectorAll(".fl-post-column").length)
        );
        ...
        console.log(`📦 ${products.length} produits trouvés sur GuizetteFamily`);
...
async function parse_results(page) {
    return await page.evaluate(() => {
        return Array.from(document.querySelectorAll(".fl-post-column")).map(el => {
            const titleElement = el.querySelector(".fl-post-grid-post a[title]");
...
(async () => {
    ...
    await checkGuizetteStock(browser);
